  ...
  return (
    <nav className="bg-white/80 backdrop-blur-xl border-b border-gray-100/50 sticky top-0 z-50 font-[Outfit]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
        ...
        {/* Desktop Menu */}
        <div className="hidden md:flex items-center gap-2 bg-slate-50/50 p-1.5 rounded-full border border-slate-100/50">
          {navLinks.map((item) => (
            <Link
              key={item.name}
              to={item.href}
            ...
            >
              <item.icon className={clsx("w-4 h-4", isActive(item.href) ? "text-blue-600" : "text-slate-400 group-hover:text-slate-600")} />
              {item.name}
            </Link>
          ))}
        </div>
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-3 md:gap-4">
            ...
            <Link
              to="/app/ats-check"
              className="hidden md:flex relative overflow-hidden text-sm font-bold items-center gap-2 px-5 py-2.5 rounded-full transition-all text-white bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 shadow-lg shadow-emerald-200/50"
            >
            ...
            <div className="relative hidden md:block">
              <button
                ...
                className="flex items-center gap-2 md:gap-3 focus:outline-none group p-1 pr-4 rounded-full border border-gray-100 bg-white hover:border-blue-100 hover:shadow-md transition-all"
              >
                ...
                <ChevronDown className={clsx("hidden md:block w-3.5 h-3.5 text-slate-400 group-hover:text-slate-600 transition-transform duration-200", profileOpen && "rotate-180")} />
              </button>
              ...
              <AnimatePresence>
                {profileOpen && (
                  <motion.div
                    ...
                    className="absolute right-0 mt-3 w-64 bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden z-50 ring-1 ring-black/5"
                  >
                    <div className="p-5 border-b border-gray-50 bg-gradient-to-br from-white to-slate-50/50">
                    ...
                    <div className="p-2">
                      <Link
                        to="/app/profile"
                        className="w-full text-left px-3 py-2.5 text-sm font-medium text-slate-600 hover:bg-indigo-50 hover:text-indigo-600 rounded-xl flex items-center gap-3 transition-colors"
